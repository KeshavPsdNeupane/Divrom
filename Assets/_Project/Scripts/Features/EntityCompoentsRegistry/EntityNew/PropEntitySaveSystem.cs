...
namespace Kope.Core.Identity {
	...
	/// </para>
	/// </remarks>
	public class PropEntitySaveSystem : MonoBehaviour, IPropEntitySavePacketProvider {
		[SerializeField] private PropInstance entityInstance;
		...
		public HashedTag UniqueID => this.entityInstance.PropEntityDetail.UniqueID.HashedTag;

		protected void Awake() {
			if (this.entityInstance == null) {
				Debug.LogError($"[PropEntitySaveSystem] EntityInstance reference is not set on" +
			...
			if (!ServiceLocator.SceneServiceLocator.Instance.TryGetService<SavableEntityRegistry>(out var savableEntityRegistry)) {
				Debug.LogError($"[PropEntitySaveSystem] No SavableEntityRegistry found in the scene." +
		...
		private void UnRegisterTheEntity(PropEntityDetail detail) {

			this._savableEntityRegistry.UnregisterEntity(this.UniqueID);
		}

		public PropEntitySavePacket GetEntitySavePacket() {
			...
			var ed = this.entityInstance.PropEntityDetail;
			return new PropEntitySavePacket(
		...
		public void LoadEntitySavePacket(PropEntitySavePacket packet) {
			var registry = this.entityInstance.ComponentsRegistryForSaveSystemOnly;
