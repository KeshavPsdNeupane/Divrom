...
public class RegionExtractionAlgorithm {
	...
	private static readonly Vec2Int[] s_neighborOffsets = {
	...
	public Dictionary<Vec2Int, List<Vec2Int>> Extract(
		...
		for (int i = 0; i < allKeys.Length; i++) {
			Vec2Int anchor = allKeys[i];
			if (visitedTiles.Contains(anchor)) continue;

			List<Vec2Int> singleRegionTiles = ExploreRegion(visitedTiles, macroTileDictionary, anchor);
	...
	private List<Vec2Int> ExploreRegion(
		...
		while (stack.Count > 0) {
			...
			for (int i = 0; i < s_neighborOffsets.Length; i++) {
				Vec2Int neighbor = currentPos + s_neighborOffsets[i];
