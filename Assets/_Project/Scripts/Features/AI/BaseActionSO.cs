...
namespace Kope.AI
{
	...
	public abstract class BaseActionSO : ScriptableObject
	{
		...
		/// </summary>
		public void Initialize(ComponentRegistry ctx)
		{
			this.actionStatus = ExecutionActionStatus.Running;
			OnInilialize(ctx);
		}
		protected abstract void OnInilialize(ComponentRegistry ctx);
		/// <summary>
		...
		/// </summary>
		public void EndOrAbort(ComponentRegistry ctx)
		{
			OnEndOrAbort(ctx);
			actionStatus = ExecutionActionStatus.NotInitialized;
			OnActionCompleted = null;
		}
		protected abstract void OnEndOrAbort(ComponentRegistry ctx);
