...
namespace Kope.Component.ExperienceSystem.Config.Editor {

	[CustomEditor(typeof(ExperienceSystemConfig))]
	public class ExperienceSystemConfigEditor : UnityEditor.Editor {
		...
		public override void OnInspectorGUI() {
			...
			ExperienceSystemConfig config = (ExperienceSystemConfig)target;
			int[] tableData = config.ExpRequiredForLevel;
			...
			testXpInput = EditorGUILayout.IntField("Enter Cumulative EXP:", testXpInput);
		...
		private void CopyTableToClipboard(int[] tableData) {
			...
			int previousExpNeed = 0;

			for (int i = 1; i < tableData.Length; i++) {
				int targetLevel = i + 1;
				int expNeed = tableData[i] - tableData[i - 1];
