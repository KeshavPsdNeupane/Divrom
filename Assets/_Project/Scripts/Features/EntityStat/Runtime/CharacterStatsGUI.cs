...
namespace Kope.Character.Stats {
	public class CharacterStatsGUI : MonoBehaviour {
		...
		private void OnDisable() {
			...
			foreach (var kvp in this.resistanceCallbacksDict)
				characterStats.ResistanceUnsubscribe(kvp.Key, kvp.Value);
			this.resistanceCallbacksDict.Clear();
		}

		private void OnGUI() {
			if (!this.canShowState) return;

			labelStyle ??= new GUIStyle(GUI.skin.label);

			labelStyle.fontSize = Mathf.RoundToInt(12 * this.scale);
			labelStyle.normal.textColor = Color.white;

			int panelWidth = Mathf.RoundToInt(250 * this.scale);
			int panelHeight = Mathf.RoundToInt(300 * this.scale);
			int startX = 10;
			int startY = 10;
			int lineHeight = Mathf.RoundToInt(20 * this.scale);
			int padding = Mathf.RoundToInt(10 * this.scale);
