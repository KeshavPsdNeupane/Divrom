...
namespace Kope.ModularSpriteAnimation.Runtime
{
    ...
    public class StaticAnimationLibraryResolver : MonoBehaviour
    {
        ...
        private void OnValidate()
        {
            ...
            {
                MyLogger.Warn($"Default Sprite Library Asset is not assigned on {this.name}.");
            }
            if (this.gender == GenderEnum.none)
            {
                MyLogger.Warn($"Gender is set to 'none' on {this.name}. This may lead to incorrect asset resolution.");
            }
            if (this.race == RacesEnum.none)
            {
                MyLogger.Warn($"Race is set to 'none' on {this.name}. This may lead to incorrect asset resolution.");
            }
